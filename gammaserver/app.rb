...
class GammaApp < Sinatra::Base
  ...
  get '/summarymeasure' do
    ...
    if ! jdata.has_key?(:id_device)
      return_message[:status] = 'error'
      return_message[:description] = "Mandatory parameter missing"
      return_message[:body] = 'id_device'
    end
    ...
    where_clause = " id_device = #{jdata[:id_device]}"
  ...
  post '/add' do
    ...
    conn.prepare("mypreparedinsert", "INSERT INTO measures (ts_device, id_device, id_measure, measured_value, max_value, min_value, baseline, location, message) " +
    "VALUES ($1::timestamp with time zone,$2::bigint,$3::bigint,$4::numeric,$5::numeric,$6::numeric,$7::numeric,ST_MakePoint($8,$9)::geography ,$10::text)")
    ...
    begin
      ...
      jdata[:baseline], jdata[:location][0], jdata[:location][1], jdata[:message]])
  ...
  def read_tuples(res,return_message)
    ...
    if res.cmd_tuples() > 0
      res.each {|tuple|
        puts "tuple " + tuple.to_s
        msg = tuple.map { |k, v| [k.to_sym, v] }.to_h
        puts "hash " + msg.to_s
#        content_type :json
        msgs.push(msg)
      }
